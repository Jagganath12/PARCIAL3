...
# ramachandran/processing.py
import pandas as pd

def process_pdb(pdb_file):
    """Lee las coordenadas atómicas del archivo PDB."""
...
def torsion_angle(a, b, c, d):
    """Calcula el ángulo de torsion (dihedral angle) entre cuatro átomos."""
...
def get_atom_coords(df, chain, residue, atom):
    """Recupera las coordenadas de un átomo específico."""
    try:
...
def classify_residue(residue):
    """Clasifica un residuo en categorías según su polaridad."""
    if residue in polar:
        return "Polar"
...
def calculate_angles(df):
    """Calcula los ángulos phi y psi para los residuos."""
    angles = []
...
# ramachandran/visualization.py
import seaborn as sns
import matplotlib.pyplot as plt

def generate_ramachandran_plots(angles_df, prot_pdb):
    """Genera los gráficos de Ramachandran."""
    ...
    sns.kdeplot(x="phi", y="psi", data=proline_angles_df, levels=3, color="darkred", fill=True, alpha=0.3, bw_adjust=0.3)
    plt.xlabel("Phi (φ)")
    plt.ylabel("Psi (ψ)")
    plt.xlim(-180, 180)
    plt.ylim(-180, 180)
    ...
    return plot_name, glycine_plot_name, proline_plot_name

# ramachandran/report.py
from fpdf import FPDF, XPos, YPos

def create_pdf(prot_pdb, plot_name, glycine_plot_name, proline_plot_name, hist_plot_name, classification_name):
...
# main.py
from ramachandran.processing import process_pdb
from ramachandran.calculations import calculate_angles
from ramachandran.visualization import generate_ramachandran_plots
from ramachandran.report import create_pdf
import os

if __name__ == "__main__":
    pdb = "data/1ewq.pdb"
    prot_pdb = pdb.split("/")[-1].split(".")[0].upper()

    df = process_pdb(pdb)
    angles_df = calculate_angles(df)

    plot_name, glycine_plot_name, proline_plot_name = generate_ramachandran_plots(angles_df, prot_pdb)

    hist_plot_name, classification_name = "hist.png", "classification.png"
    create_pdf(prot_pdb, plot_name, glycine_plot_name, proline_plot_name, hist_plot_name, classification_name)

    for file in [plot_name, glycine_plot_name, proline_plot_name, hist_plot_name, classification_name]:
        os.remove(file)
